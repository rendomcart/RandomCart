...
const Chatbot = () => {
  ...
  const welcomeMessage = {
    id: 1,
    sender: 'bot',
    text: 'Hello Admin 👋, need help navigating the dashboard?',
  };
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      const { data } = await axios.get(`/faqs?search=${encodeURIComponent(text.trim())}&all=true`);
      
      let botText = "I'm sorry, I couldn't find an answer in the FAQs.";
      ...
      >
        <div className="bg-primary text-white p-4 rounded-t-xl flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm leading-tight">Admin Assistant</h3>
              <p className="text-xs text-blue-100">Quick FAQ Search</p>
            </div>
          </div>
