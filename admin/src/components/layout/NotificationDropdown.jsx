...
const NotificationDropdown = ({ isOpen, onClose }) => {
  ...
  return (
    <div 
      ref={dropdownRef}
      className="absolute top-12 right-0 w-80 sm:w-96 bg-white rounded shadow-xl border border-gray-200 z-50 flex flex-col max-h-[80vh] overflow-hidden"
      ...
      <div className="overflow-y-auto flex-grow bg-white">
        {notifications.length === 0 ? (
          <div className="p-8 text-center text-gray-500 flex flex-col items-center">
            ...
            <p>System is all clear. No alerts.</p>
          </div>
        ) : (
          <ul className="divide-y divide-gray-100">
            {notifications.map((notif) => (
              <li 
                ...
                className={`p-4 hover:bg-gray-50 transition-colors relative group cursor-pointer ${!notif.isRead ? 'bg-red-50/30' : ''}`}
              >
                {!notif.isRead && (
                  <span className="absolute top-4 left-2 w-2 h-2 bg-red-500 rounded-full"></span>
                )}
                <div className="pl-4 pr-2 sm:pr-6">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-1 gap-1 sm:gap-0">
                    ...
                    <span className="text-[10px] text-gray-400 whitespace-nowrap sm:ml-2">
